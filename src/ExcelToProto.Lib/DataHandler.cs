...
namespace HiProtobuf.Lib
{
    internal class DataHandler
    {
        ...
        {
            System.Diagnostics.Debug.Assert(File.Exists(path), "Excel file can not find");
                ...
                for (int i = 4; i <= rowCount; i++)
                {
                    ...
                    for (int j = 1; j <= columnCount; j++)
                    {
                        ...
                        if (insField == null)
                        {
                            ...
                            if (insField == null)
                            {
                               Helper.LogInfo($"文件： {name} 属性： {variableName} 没有反射获取到对应的数据");
                            }
                            Helper.LogInfo($"文件： {name} 属性： {variableName} 命名规则不正常，注意修复");
                        }
        ...
        object GetVariableValue(string type, string value)
        {
            ...
            Helper.LogError($"type: {type}  value: {value}");
            return null;
        }
